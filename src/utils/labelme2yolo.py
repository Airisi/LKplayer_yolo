...
class Labelme2YOLO(object):
    ...
    def _save_yolo_image(self, json_data, json_name, image_dir_path, target_dir):
        ...
        支持两种来源：
        1. json 内嵌 imageData（base64）
        2. 外部 imagePath 文件

        :param json_data: labelme json 数据
        :param json_name: json 文件名
        :param image_dir_path: 图片输出目录
        :param target_dir: train/val 子目录
        :return: 保存后的图片路径
        """
        img_name = json_name.replace('.json', '.png')
        img_path = os.path.join(image_dir_path, target_dir, img_name)
        ...
        if not os.path.exists(img_path):
            ...
            if json_data.get('imageData'):
                try:
                    ...
                    print(f"[ERROR] base64 decode failed: {json_name}, {e}")
                    return None

            # 情况2：从 imagePath 读取
            elif json_data.get('imagePath'):
                src_path = os.path.join(self._json_dir, json_data['imagePath'])

                if not os.path.exists(src_path):
                    print(f"[ERROR] imagePath not found: {src_path}")
                    return None

                shutil.copy(src_path, img_path)

            else:
                print(f"[ERROR] No imageData or imagePath in {json_name}")
                return None
